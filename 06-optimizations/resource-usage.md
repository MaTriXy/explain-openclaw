...
| 4 | **Plugin loading via jiti** — synchronous TypeScript transpilation per plugin at startup | `src/plugins/loader.ts:166-195` | High (startup) | Like compiling a recipe book from scratch every time you open the kitchen, instead of using a pre-printed copy |
...
| 9 | **Media understanding** — sending media to AI providers (Whisper/Gemini/OpenAI) for transcription | `src/media-understanding/runner.ts:773-919` | Medium | CPU cost is mostly on the provider side, but local buffering and encoding still takes cycles |
...
| 13 | **Agent execution loop** — continuous model response processing | `src/auto-reply/reply/agent-runner-execution.ts:139` | Medium (continuous) | The main "brain" loop — always running while the bot is responding |
| 14 | **Cron timer loop** — re-arming `setTimeout` for scheduled job processing | `src/cron/service/timer.ts:638` | Low (idle) | Like a clock ticking in the background — minimal CPU unless jobs are firing |
...
- `src/process/exec.ts:339-342` — unbounded string concatenation of process output
...
| SQLite databases | `extensions/memory-core/src/memory/manager-sync-ops.ts:88-98` | **No VACUUM** — database grows unbounded without periodic vacuuming (uses default DELETE journal mode, no WAL) |
