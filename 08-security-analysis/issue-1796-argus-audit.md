...
| 6 | Path traversal in agent dirs | **False** | Paths go through `resolveUserPath()` (`src/agents/agent-paths.ts:9,12`) which calls `path.resolve()` (`src/infra/home-dir.ts:112,114`), normalizing traversal. IDs from env/config, not user input. |
